...
const AddAddress: React.FC<IMultisigProps> = ({ addAddress, onCancel, setAddAddress, className }) => {
	const { addressBook, activeMultisig, multisigAddresses, setUserDetailsContextState, identityBackend } = useGlobalUserDetailsContext();
	...
	const handlePersonalAddressBookUpdate = async () => {
		...
		try {
			...
			const { data: addAddressResponse, error: addAddressError } = await identityBackend.addAddressToAddressBook(ownerAddress, {
				address,
				discord,
				email,
				name,
